...
class HeadlineClassification(pl.LightningModule) :
    def __init__(self, config) :
        ...
        self.metric_acc = torchmetrics.Accuracy(task="multiclass", num_classes=3, average='macro')
        self.metric_f1 = torchmetrics.F1Score(task="multiclass", num_classes=3, average='macro')
    ...
    def step(self, batch, batch_idx, state=None) :
        label = batch['labels'].to(device) # label shape : [batch]
