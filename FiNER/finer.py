    ...
    def __init__(self):
        ...
        if Configuration['task']['model'] == 'bilstm':
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(
                pretrained_model_name_or_path=self.train_params['tokenizer_name'],
                            ...
                            if re.fullmatch(r'(\d+[\d,.]*)|([,.]\d+)', sample_tokens[sample_idx][token_idx]):
                                if self.train_params['replace_numeric_values'] == 'NUM':
                                    ...
                                    if shape in self.shape_special_tokens_set:
                                        ...
                                        sample_tokens[sample_idx][token_idx] = '[NUM]'
                                elif self.train_params['replace_numeric_values'] == 'RANDOM':
                                    token = sample_tokens[sample_idx][token_idx]
                                    replaced_token = ''
                                    for char in token:
                                        if char.isdigit():
                                            replaced_token += str(random.randint(0, 9))
                                        else:
                                            replaced_token += char
                                    sample_tokens[sample_idx][token_idx] = replaced_token
