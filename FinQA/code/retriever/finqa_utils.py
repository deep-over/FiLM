...
# sys.path.insert(0, '../utilss/')
sys.path.append((os.path.dirname(os.path.abspath(os.path.dirname('/home/jaeyoung/FinQA/code/utilss/general_utils')))))
...
def tokenize(tokenizer, text, apply_basic_tokenization=False):
    ...
    #     _SPECIAL_TOKENS_RE = re.compile(r"(\d+[\d,.]*)|([,.]\d+)", re.UNICODE)

    if conf.pretrained_model in ["bert", "finbert", "sec-bert"]:
        _SPECIAL_TOKENS_RE = re.compile(r"^\[[^ ]*\]$", re.UNICODE)
    elif conf.pretrained_model in ["roberta", "longformer", "roberta-retrained"]:
        _SPECIAL_TOKENS_RE = re.compile(r"^<[^ ]*>$", re.UNICODE)
